...
//	var VELOCITY_SCALE = 0.005 * (Math.pow(window.devicePixelRatio, 1 / 3) || 1); // scale for wind velocity (completely arbitrary--this value looks nice)
	var VELOCITY_SCALE = 0.006 * (Math.pow(window.devicePixelRatio, 1 / 3) || 1); // scale for wind velocity (completely arbitrary--this value looks nice)
...
//	var MAX_TEMPERATURE_K = 317.15; // wind velocity at which particle intensity is maximum (m/s)
	var MIN_TEMPERATURE_K =  99500; // step size of particle intensity color scale
	var MAX_TEMPERATURE_K = 102800; // wind velocity at which particle intensity is maximum (m/s)
...
//	var PARTICLE_MULTIPLIER = 1 / 200; // particle count scalar (completely arbitrary--this values looks nice)
	...
	var createWindBuilder = function createWindBuilder(uComp, vComp, press) {
		...
		return {
			...
			data: function data(i) {
				return [uData[i], vData[i], press.data[i]];
			},
	...
	var createBuilder = function createBuilder(data) {
		...
		return createWindBuilder(uComp, vComp, press);
	};
...
*/
			var result = ["rgb(36,104, 180)", "rgb(60,157, 194)", "rgb(128,205,193 )", "rgb(151,218,168 )", "rgb(198,231,181)", "rgb(255,182,100)", "rgb(252,150,75)", "rgb(250,112,52)", "rgb(245,64,32)", "rgb(237,45,28)", "rgb(220,24,32)", "rgb(180,0,35)", "rgb(255,255,255)"];
...
//			htmlOut = "<strong>Wind Direction: </strong>" + self.vectorToDegrees(gridValue[0], vMs) + "°" + ", <strong>Wind Speed: </strong>" + self.vectorToSpeed(gridValue[0], vMs).toFixed(1) + "m/s" + ", <strong>Temp: </strong>" + (gridValue[2] - 273.15).toFixed(1) + "°C";
			htmlOut = "<strong>Wind Direction: </strong>" + self.vectorToDegrees(gridValue[0], vMs) + "°" + ", <strong>Wind Speed: </strong>" + self.vectorToSpeed(gridValue[0], vMs).toFixed(1) + "m/s" + ", <strong>Pressure: </strong>" + (gridValue[2]/100).toFixed(1) + "hPa";
		...
		if (jQuery('.leaflet-control-wind-position').index() == 0) {
			jQuery('.leaflet-control-wind-position').insertAfter('.leaflet-control-mouseposition');
		}
	}
...
var WindJSHelper = {
	...
	getRequestUrl: function getRequestUrl() {
		...
		return WindJSHelper.options.nearestUrl + jQuery.param(params);
	},
	...
	loadLocalData: function loadLocalData() {
		...
		jQuery.getJSON('demo.json', function (data) {
...
var WindJSLeaflet = function WindJSLeaflet(options) {
	...
	checkWind(options).then(function () {
        ...
        if (options.layerControl != null)
		    options.layerControl.addOverlay(WindJSHelper.canvasOverlay, 'Wind');
        WindJSHelper.canvasOverlay.addTo(WindJSHelper.map);
	}).catch(function (err) {
